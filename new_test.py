...
class CatBreakReminder(QMainWindow):
    def __init__(self):
        ...
        self.window_width, self.window_height = 404, 445
        ...
        self.cat_label.setGeometry(137, 53, self.window_width, self.window_height)
    ...
    def wheelEvent(self, event):
        delta = event.angleDelta().y()  # Get the scroll amount (positive for scroll up, negative for scroll down)

        zoom_factor = 1.1 if delta > 0 else 0.9  # Zoom in if scrolling up, zoom out if scrolling down
        self.change_window_size(zoom_factor)

    def change_window_size(self, scale):
        rect = self.geometry()
        ...
        self.update_layout(new_width, new_height, scale)

    def update_layout(self, new_width, new_height, scale):
        # Update geometry of the background and cat images
        self.background_label.setGeometry(0, 0, new_width, new_height)
        
        # Update cat's geometry while maintaining its relative position on top of the house
        cat_x = int(137 * scale)  # Cast to int for geometry
        cat_y = int(53 * scale)   # Cast to int for geometry
        cat_width = int(144 * scale)  # Cast to int for geometry
        cat_height = int(144 * scale)  # Cast to int for geometry
        
        # Set the new geometry for the cat label
        self.cat_label.setGeometry(cat_x, cat_y, cat_width, cat_height)
        
        # Update status and timer label positions (they are not zoomable but should maintain their position)
